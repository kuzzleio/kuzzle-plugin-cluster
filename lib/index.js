...
class KuzzleCluster {
  ...
  connectedToLB () {
    ...
    console.log(`${new Date().toISOString()} [info] ${log}`);   // eslint-disable-line no-console
  ...
  clusterStatusAction () {
    return Promise.resolve(this.node.clusterStatus);
  }
...
function onLbMessage (msg) {
  ...
  if (msg.action === 'joined') {
    ...
    let log = `[cluster] ACK for ${msg.on} event received from LB`;
    // at start, the logger is not ready, use console in doubt
    this.kuzzle.pluginsManager.trigger('log:info', log);
    console.log(`${new Date().toISOString()} [info] ${log}`);   // eslint-disable-line no-console
  }
}
...
function onJoinedLb (msg) {
  ...
  return this.node.init()
    .then(() => {
      ...
      log = '[cluster] Notification: Kuzzle is ready';
      this.kuzzle.pluginsManager.trigger('log:info', log);
      console.log(`${new Date().toISOString()} [info] ${log}`);   // eslint-disable-line no-console

      log = `[cluster] ${this.uuid} joined as ${this.node.constructor.name} on ${msg.host}:${msg.port}`;
      this.kuzzle.pluginsManager.trigger('log:info', log);
      console.log(`${new Date().toISOString()} [info] ${log}`);   // eslint-disable-line no-console
