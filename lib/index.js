...
class KuzzleCluster {
  ...
  clusterHealthAction () {
    if (!this.node.ready) {
      throw new this.context.errors.NotFoundError('ko');
    }
  ...
  setRoomCount (index, collection, roomId, count) {
  ...
  _onShutDown (event) {
    ...
    if (Object.keys(this.node.pool) <= 1) {
  ...
  _realtimeCountOverride (request, attempt = 0) {
    ...
    return {count: this._rooms.flat[roomId].count};
  }
