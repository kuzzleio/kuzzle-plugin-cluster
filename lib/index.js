...
KuzzleCluster.prototype.kuzzleStarted = function () {
  ...
  this.lbBroker.listen('cluster:' + this.uuid, resetNode.bind(this));
  this.lbBroker.listen('cluster:master', resetNode.bind(this));
...
function resolveBinding (config) {
  var
    host = config,
    port = _context.accessors.kuzzle.config.internalBroker.port,
    match,
    iface,
    family,
    tmp;
...
function resetNode (msg) {
  ...
  if (msg.uuid === this.uuid) {
    ...
    this.node = new SlaveNode(_context, {
      host: msg.host,
      port: msg.port,
  ...
  return this.node.init()
    ...
    .catch(err => {
      ...
      this.lbBroker.send('cluster:' + this.uuid, {
        error: {
          code: 2,
