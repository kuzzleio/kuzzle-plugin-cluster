...
class KuzzleCluster {
  ...
  _onLbMessage (msg) {
    ...
    debug('[%s] received message from load balancer: %a', this.uuid, parsed);

    if (parsed.action === 'joined') {
      return this._onJoinedLb(parsed);
    }
  ...
  _onJoinedLb (msg) {
    ...
    if (msg.uuid === this.uuid) {
      ...
      this.node = new MasterNode(this, this.context, {});
    }
    else {
      this.isMasterNode = false;
      this.node = new SlaveNode(this, this.context, {
        host: msg.host,
        port: msg.port,
    ...
    return this.node.init()
      ...
      .timeout(this.config.initTimeout, `Could not init cluster node within ${this.config.initTimeout}ms`)
      .catch(err => {
   ...
   */
  _resolveBinding (hostConfig = this.config.binding) {
    let
      host = hostConfig,
      port = this.kuzzle.config.services.internalBroker.port,
      match,
      iface,
      family,
      tmp;

    debug('resolving broker bindings with configuration: %a', host);
