...
class Node {
  ...
  init () {
    ...
    return Bluebird.all([
      ...
      .then(() => {
        setInterval(() => this.broadcast('cluster:heartbeat', this), this.config.timers.heartbeat);
        return this.join();
      });
  }
  ...
  sync (data) {
    ...
    switch (data.event) {
      ...
      case 'indexCache:add':
        return this.kuzzle.indexCache.add(data.index, data.collection, false);
      case 'indexCache:remove':
        return this.kuzzle.indexCache.remove(data.index, data.collection, false);
      case 'indexCache:reset':
        return this.kuzzle.indexCache.reset();
      case 'profile':
