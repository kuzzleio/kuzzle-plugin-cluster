...
class Node {
  ...
  discover () {
    ...
    return this.redis.sadd('cluster:discovery', JSON.stringify({
    ...
    switch (data.event) {
      ...
      case 'strategy:added':
        try {
          ...
          // log & discard
          this.context.log('error', `Plugin ${data.pluginName} - tried to add the strategy "${data.name}": ${e.message}`);
        }
        break;
      case 'strategy:removed':
        try {
          ...
          // log & discard
          this.context.log('error', `Plugin ${data.pluginName} - tried to remove the strategy "${data.name}": ${e.message}`);
        }
        break;
      case 'state':
  ...
  _onRouterMessage (envelope, buffer) {
    ...
    // called from a client to force rejoin, for instance if a heartbeat is received back
  ...
  _onSubMessage (buffer) {
    ...
    if (room === 'cluster:heartbeat') {
      ...
      this.kuzzle.repositories.profile.profiles = {};
      this.kuzzle.repositories.role.roles = {};
    }
  }
