...
class Node {
  ...
  init () {
    ...
    return Bluebird.all([
      ...
      .then(() => {
        setInterval(() => this.broadcast('cluster:heartbeat', this.config.bindings.pub.href), this.config.timers.heartbeat);
        return this.join();
      });
  }
  ...
  /**
   * Add a cluster node the list of of known siblings
   *
   * @param {object} node
   * @private
   */
   ...
   *
   * @param {string} nodePub - node uuid
   * @private
   */
  _heartbeat (nodePub) {
    ...
    node.heartbeat = setTimeout(() => {
      this.context.log('warn', `[cluster] no heartbeat received in time for ${nodePub}. removing node`);
  ...
  _onRouterMessage (envelope, buffer) {
    ...
    if (action === 'remoteSub') {
      if (!this.pool[data.pub]) {
        this._addNode(data);
        this.pool[data.pub].uuid = data.uuid;
      }
