...
class StateManager {
  ...
  reset () {
    ...
    const scan = (node, cursor) => {
      ...
      return node.scan(cursor, 'MATCH', 'cluster*', 'COUNT', 1000)
        .then(response => {
          const keys = response.keys.filter(key => key !== 'cluster:discovery');
  ...
  sync (data) {
    ...
    return State.current(this.node.redis, index, collection)
      .then(state => {
        ...
        // autorefresh
        for (const k in state.autorefresh) {
  ...
  syncAll (data) {
    ...
    return this.node.redis
      ...
      .then(() => {
        this.node.sync({event: 'autorefresh'});

        // prevent bluebird warnings about the promise chain not
        // returning anything
        return null;
      });
  }
