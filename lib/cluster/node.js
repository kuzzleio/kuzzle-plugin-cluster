...
let
  debug = require('../kuzzleDebug')('kuzzle:cluster:node'),
  debugMerge = require('../kuzzleDebug')('kuzzle:cluster:node:merge'),
 ...
 * @class Node
 *
 * @property {KuzzleCluster} clusterHandler
 * @property {PluginContext} context
 * @property {object} options
 * @property {?ProxyBroker} broker
 * @property {Kuzzle} kuzzle
 * @property {object} clusterStatus
 * @property {object} isReady
 */
class Node {
  /**
  ...
  detach () {
    debug('detaching node from cluster broker');
  ...
  merge (diffs) {
    debug('cluster updated, need to merge diffs: %a', diffs);
    ...
    for (const diff of diffs) {
      ...
      for (const k of keys) {
        switch (k) {
          // indexCache::add
          case 'icAdd':
            debugMerge('merging data "indexCache::add": %a', diff.icAdd);
            this.kuzzle.indexCache.add(diff.icAdd.i, diff.icAdd.c, false);
            break;
          // indexCache::remove
          case 'icDel':
            debugMerge('merging data "indexCache::remove": %a', diff.icDel);
            this.kuzzle.indexCache.remove(diff.icDel.i, diff.icDel.c, false);
            break;
          // indexCache::reset
          case 'icReset':
            debugMerge('merging data "indexCache::reset": %a', diff.icReset);
            this.kuzzle.indexCache.reset(diff.icReset.i, false);
            break;
          case 'hcR':
            debugMerge('merging data "hotelClerk::addRoom": %a', diff.hcR);
            this.mergeAddRoom(diff.hcR);
            break;
          case 'hcDel':
            debugMerge('merging data "hotelClerk::deleteRoom": %a', diff.hcDel);
            this.mergeDelRoom(diff.hcDel);
            break;
          // dsl
          case 'ftAdd':
            debugMerge('merging data "filterTree::add": %a', diff.ftAdd);
            this.kuzzle.dsl.storage.store(diff.ftAdd.i, diff.ftAdd.c, diff.ftAdd.f);
            break;
          // autoRefresh
          case 'ar':
            debugMerge('merging data "index::autoRefresh": %a', diff.ar);
            this.updateAutoRefresh(diff.ar.i, diff.ar.v);
            break;
          // validation specifications update
          case 'vu':
            debugMerge('merging data "specifications::update": %a', '<empty data>');
            this.kuzzle.validation.curateSpecification();
            break;
          // cluster status
          case 'cs':
            debugMerge('merging data "cluster::status": %a', diff.cs);
            this.clusterStatus = diff.cs;
            break;
          // profile delete & update
          case 'secPU':
            debugMerge('merging profile "%s": %a', k, diff[k]);
            delete this.kuzzle.repositories.profile.profiles[diff[k]._id];
            break;
          // role delete & update
          case 'secRU':
            debugMerge('merging role "%s": %a', k, diff[k]);
  ...
  mergeAddRoom (diff) {
    ...
    if (!hotelClerk.customers[connection.i] || !hotelClerk.customers[connection.i][roomId]) {
      hotelClerk._addRoomForCustomer(request, roomId, metadata);
    }
  }
  ...
  mergeDelRoom (diff) {
    ...
    if (hotelClerk.rooms[diff.r]) {
      hotelClerk._removeRoomForCustomer(context, diff.r, false);
    }
  }
