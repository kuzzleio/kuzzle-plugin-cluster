...
class SlaveNode extends Node {
  ...
  attachEvents () {
    ...
    this.broker.listen(`cluster:${this.clusterHandler.uuid}`, msg => {
      debug('private message received from master node: %a', msg);

      switch (msg.action) {
        case 'snapshot':
          this.kuzzle.hotelClerk.rooms = this._unserializeRooms(msg.data.hc.r);
  ...
  _unserializeRooms (serialized) {
    const rooms = {};

    for (const roomId of Object.keys(serialized)) {
      const room = serialized[roomId];
